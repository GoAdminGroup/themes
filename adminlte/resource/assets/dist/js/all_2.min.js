...
removeActive();if(content===""){let sidebarMenus=$('.sidebar-menu a');let re=new RegExp("\\?(.*)");for(let i=0;i<sidebarMenus.length;i++){console.log("link",link,"$(sidebarMenus[i]).attr('href')",$(sidebarMenus[i]).attr('href'));if(link.replace(re,'')===$(sidebarMenus[i]).attr('href')){content=$(sidebarMenus[i]).html().replace('<i/><span>','<i/>&nbsp&nbsp&nbsp<span>');break}}}
...
function checkNavExist(link){let navs=$('.nav-addtabs li');for(let i=0;i<navs.length;i++){if($(navs[i]).find('a').attr('href')===link){removeActive();$(navs[i]).addClass('active');return true;}}
return false;}
