...
removeActive();addNavTab(link,$(this).html().replace('<i/><span>','<i/>&nbsp&nbsp&nbsp<span>'))}});function listenerForAddNavTab(link){let content='';if(link!=='#'&&link.indexOf('http')===-1&&!checkNavExist(link)){if(!checkNavLength()){removeFirst();}
removeActive();let sidebarMenus=$('.sidebar-menu a');for(let i=0;i<sidebarMenus.length;i++){if(link.indexOf($(sidebarMenus[i]).attr('href'))!==-1){content=$(sidebarMenus[i]).html().replace('<i/><span>','<i/>&nbsp&nbsp&nbsp<span>');break}}
