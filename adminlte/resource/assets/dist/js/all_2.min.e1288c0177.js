...
NProgress.done();updateNavURL();});let fullpageBtn=$('.fullpage-btn');let exitFullpageBtn=$('.exit-fullpage-btn');fullpageBtn.unbind('click').on('click',function(){launchFullscreen(document.documentElement);fullpageBtn.hide();exitFullpageBtn.show();});exitFullpageBtn.unbind('click').on('click',function(){exitFullscreen();exitFullpageBtn.hide();fullpageBtn.show();});function launchFullscreen(element){if(element.requestFullscreen){element.requestFullscreen();}else if(element.mozRequestFullScreen){element.mozRequestFullScreen();}else if(element.msRequestFullscreen){element.msRequestFullscreen();}else if(element.webkitRequestFullscreen){element.webkitRequestFullScreen();}}
...
$('.container-refresh').unbind('click').on('click',function(){$.pjax.reload('#pjax-container');toastr.success(toastMsg);});let sidebarMenuA=$('.sidebar-menu a');$(function(){$('.sidebar-menu li:not(.treeview) > a').unbind('click').on('click',function(){let parent=$(this).parent().addClass('active');parent.siblings('.treeview.active').find('> a').trigger('click');parent.siblings().removeClass('active').find('li').removeClass('active');});$('[data-toggle="popover"]').popover();addOrRemoveLeftRightNavBtn(false);initMaxNavWrapperWidth();});$(window).resize(function(){initMaxNavWrapperWidth();addOrRemoveLeftRightNavBtn(!checkNavLength());});sidebarMenuA.unbind('click').unbind('click').on('click',function(){let link=$(this).attr('href');if(link!=='#'&&link.indexOf('http')===-1&&!checkNavExist(link)){addOrRemoveLeftRightNavBtn(!checkNavLength());removeActive();addNavTab(link,$(this).html().replace('<i/><span>','<i/>&nbsp&nbsp&nbsp<span>'))
moveToRight();}});$('a.new-tab-link').unbind('click').on('click',function(){listenerForAddNavTab($(this).attr('href'),$(this).attr('data-title'))});$('.navbar-nav-btn-left').unbind('click').on('click',function(){moveToLeft();});function moveToLeft(){let nav=$(".nav.nav-tabs.nav-addtabs");let marginLeft=parseInt(nav.css("marginLeft"));if(marginLeft<0){if(marginLeft<-50&&marginLeft>-100){nav.css("marginLeft","0px");}else{nav.css("marginLeft",(marginLeft+50)+"px");}}}
$('.navbar-nav-btn-right').unbind('click').on('click',function(){moveToRight();});function moveToRight(){let nav=$(".nav.nav-tabs.nav-addtabs");let marginLeft=parseInt(nav.css("margin-left"));let ulWidth=getNavULwidth();let padding=ulWidth-maxNavWrapperWidth;if(padding>0&&marginLeft+padding!=0){if(padding+marginLeft<100){nav.css("margin-left",-padding+"px");}else{nav.css("margin-left",(marginLeft-50)+"px");}}}
...
const fixedKey="go_admin__sidebar_fixed";$(function(){let isFixed=window.localStorage.getItem(fixedKey);if(isFixed==="true"){$('.main-sidebar').css('position','fixed');$('.main-header .logo').css('position','fixed');$('.fixed-btn').attr('data-click','true');}});$('.fixed-btn').unbind('click').on('click',function(){let clicked=$(this).attr('data-click');if(clicked==="false"){$('.main-sidebar').css('position','fixed');$('.main-header .logo').css('position','fixed');$(this).attr('data-click','true');window.localStorage.setItem(fixedKey,"true");$(this).css('background-color','#f3f3f3');}else{$('.main-sidebar').css('position','');$('.main-header .logo').css('position','');$(this).attr('data-click','false');window.localStorage.removeItem(fixedKey);$(this).css('background-color','white');}});