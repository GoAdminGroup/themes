...
removeActive();addNavTab(link,$(this).html().replace('<i/><span>','<i/>&nbsp&nbsp&nbsp<span>'))}});$('a.new-tab-link').on('click',function(){listenerForAddNavTab($(this).attr('href'))});function listenerForAddNavTab(link){let content='';if(link!=='#'&&link.indexOf('http')===-1&&!checkNavExist(link)){if(!checkNavLength()){removeFirst();}
removeActive();let sidebarMenus=$('.sidebar-menu a');let re=new RegExp("\\?(.*)");for(let i=0;i<sidebarMenus.length;i++){if(link.replace(re,'')===$(sidebarMenus[i]).attr('href')){content=$(sidebarMenus[i]).html().replace('<i/><span>','<i/>&nbsp&nbsp&nbsp<span>');break}}
