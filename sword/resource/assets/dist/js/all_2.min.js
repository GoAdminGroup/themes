...
$('.container-refresh').on('click',function(){$.pjax.reload('#pjax-container');toastr.success(toastMsg);});let sidebarMenuA=$('.sidebar-menu a');$(function(){$('.sidebar-menu li:not(.treeview) > a').on('click',function(){let parent=$(this).parent().addClass('active');parent.siblings('.treeview.active').find('> a').trigger('click');parent.siblings().removeClass('active').find('li').removeClass('active');});$('[data-toggle="popover"]').popover();});sidebarMenuA.on('click',function(){let link=$(this).attr('href');if(link!=='#'&&link.indexOf('http')===-1&&!checkNavExist(link)){if(!checkNavLength()){removeFirst();}
...
removeActive();if(content===""){let sidebarMenus=sidebarMenuA;let re=new RegExp("\\?(.*)");for(let i=0;i<sidebarMenus.length;i++){if(link.replace(re,'')===$(sidebarMenus[i]).attr('href')){content=$(sidebarMenus[i]).html().replace('<i/><span>','<i/>&nbsp&nbsp&nbsp<span>');break}}}
